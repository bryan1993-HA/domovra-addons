...
from db import (
    # affichage
    list_locations, list_lots, list_products,
    status_for,
    # actions
...
def locations_page(request: Request):
    ...
    # Compteurs par emplacement (même logique qu'avant)
...
# --------- Endpoints d’actions (identiques à l’existant) ---------
...
def location_delete(request: Request, location_id: int = Form(...), move_to: str = Form("")):
    ...
    if move_to_id:
        try:
            ...
            # on log seulement, la suppression continue quand même
